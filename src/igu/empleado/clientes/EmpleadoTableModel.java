...
public class EmpleadoTableModel extends AbstractTableModel {
    ...
    public EmpleadoTableModel() {
        lis = EmpleadoData.list("");
    }

    public EmpleadoTableModel(String filter) {
        lis = EmpleadoData.list(filter);
    }

    @Override
    public Object getValueAt(int row, int column) {
        Empleado d = (Empleado) lis.get(row);
        switch (column) {
            case 0:
                return row + 1;
            case 1:
...
*/
    ...
    public void addRow(Empleado d) { // con db no se usa
        this.lis.add(d);
